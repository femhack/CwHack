...
public class TriggerBotFeature extends Feature implements UpdateListener
{
	...
	private DecimalSetting cooldown = new DecimalSetting("cooldown", "the required cooldown progress for the next attack", 1);

	private BooleanSetting attackInAir = new BooleanSetting("attackInAir", "whether or not to attack target when they are in mid air", true);
	private BooleanSetting attackOnJump = new BooleanSetting("attackOnJump", "whether or not to attack when you are jumping", true);
